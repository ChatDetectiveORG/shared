...
func TestEmitMessageUserAnswerReturnsUserMessage(t *testing.T) {
	jobs := make(chan *publishEnvelope, 1)
	waiters := &sync.Map{}
	hashe := HandlerChainHashe{}.Init(jobs, waiters, "run-5")
	userMessage := &tele.Message{ID: 21, Chat: &tele.Chat{ID: 777}}
	...
	go func() {
		...
		replyCh <- &SendResult{
			CorrelationID: job.correlationID,
			IsSuccess:     true,
			SentMessage:   &tele.Message{ID: 22, Chat: &tele.Chat{ID: 777}},
		}
	}()
	...
	got, err := hashe.EmitMessageUserAnswer(ctx, "telegram.message.send", &tele.Message{
		Text:    "answer",
		ReplyTo: userMessage,
	})
	if e.IsNonNil(err) {
		t.Fatalf("emit user answer: %v", err)
	}
	if got != userMessage {
		t.Fatalf("expected user message %#v, got %#v", userMessage, got)
	}
	...
	if request.Message == nil || request.Message.Chat == nil || request.Message.Chat.ID != userMessage.Chat.ID {
		t.Fatalf("expected answer chat to be filled from user message, got %#v", request.Message)
	}
}
